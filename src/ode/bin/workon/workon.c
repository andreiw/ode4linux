...
#define  ECHO         "/bin/echo"                          /* echo command */
    ...
    UIINIT init [] = {                      /* initialize the user interface */
		...
		{ ON_OP,		1, OVERWRITE, 1, 1, ARGS_OP },
		{ ON_OS_OP,		1, OVERWRITE, 1, 1, ARGS_OP },
		{ ON_OBJFMT_OP,		1, OVERWRITE, 1, 1, ARGS_OP },
		{ TARGET_OP,		1, OVERWRITE, 1, 1, ARGS_OP },
		{ TARGET_OS_OP,		1, OVERWRITE, 1, 1, ARGS_OP },
		{ TARGET_OBJFMT_OP,	1, OVERWRITE, 1, 1, ARGS_OP },
    };
...

{
  ...
  cp = ui_arg_value(ON_OP, 1, 1);
  if (cp != NULL && setenv("ode_on", cp, TRUE) == ERROR)
      uquit(ERROR, FALSE, "\tode_on setenv failure.\n");
  cp = ui_arg_value(ON_OS_OP, 1, 1);
  if (cp != NULL && setenv("ode_on_os", cp, TRUE) == ERROR)
      uquit(ERROR, FALSE, "\tode_on_os setenv failure.\n");
  cp = ui_arg_value(ON_OBJFMT_OP, 1, 1);
  if (cp != NULL && setenv("ode_on_objfmt", cp, TRUE) == ERROR)
      uquit(ERROR, FALSE, "\tode_on_objfmt setenv failure.\n");
  cp = ui_arg_value(TARGET_OP, 1, 1);
  if (cp != NULL && setenv("ode_target", cp, TRUE) == ERROR)
      uquit(ERROR, FALSE, "\tode_target setenv failure.\n");
  cp = ui_arg_value(TARGET_OS_OP, 1, 1);
  if (cp != NULL && setenv("ode_target_os", cp, TRUE) == ERROR)
      uquit(ERROR, FALSE, "\tode_target_os setenv failure.\n");
  cp = ui_arg_value(TARGET_OBJFMT_OP, 1, 1);
  if (cp != NULL && setenv("ode_target_objfmt", cp, TRUE) == ERROR)
      uquit(ERROR, FALSE, "\tode_target_objfmt setenv failure.\n");
