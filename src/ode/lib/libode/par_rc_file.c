 ...
 * HISTORY
 * $Log: par_rc_file.c,v $
 * Revision 1.9.11.1  1994/03/09  20:24:43  marty
 * 	CR # 1038 - Change warnings to errors.
 * 	[1994/03/09  20:24:30  marty]
 *
...
parse_single_rc_file( FILE *rcfile, struct rcfile *rcfile_p )
{
    ...
    int isreplace;
    int skip;
    const char *host_machine_name, *host_os_name, *host_objfmt_name;
    const char *target_machine_name, *target_os_name, *target_objfmt_name;
	...
	for (;;) {
		...
		host_machine_name = getenv("ode_on");
		if (host_machine_name == NULL)
		    host_machine_name = MACHINE;
		if (strcmp(arg, host_machine_name) != 0) {
		    skip = TRUE;
		    break;
		}
		continue;
	    ...
	    if (strcmp(field, "on_os") == 0) {
		arg = nxtarg(&p, " \t");
		if (*arg == '\0') {
		    fprintf(stderr, "Missing os name for on_os directive\n");
		    skip = TRUE;
		    break;
		}
		host_os_name = getenv("ode_on_os");
		if (host_os_name == NULL)
		    host_os_name = OS;
		if (strcmp(arg, host_os_name) != 0) {
		    skip = TRUE;
		    break;
		}
		continue;
	    ...
	    if (strcmp(field, "on_objfmt") == 0) {
		arg = nxtarg(&p, " \t");
		if (*arg == '\0') {
		    fprintf(stderr, "Missing object format name for on_objfmt directive\n");
		    skip = TRUE;
		    break;
		}
		host_objfmt_name = getenv("ode_on_objfmt");
		if (host_objfmt_name == NULL)
		    host_objfmt_name = "<undef>";
		if (strcmp(arg, host_objfmt_name) != 0) {
		    skip = TRUE;
		    break;
		}
		continue;
		...
		arg = nxtarg(&p, " \t");
		if (*arg == '\0') {
		    fprintf(stderr, "Missing machine name for target directive\n");
		    skip = TRUE;
		    break;
		}
		target_machine_name = getenv("ode_target");
		if (target_machine_name == NULL)
		    target_machine_name = "<undef>";
		if (strcmp(arg, target_machine_name) != 0) {
		    skip = TRUE;
		    break;
		}
		continue;
	    ...
	    if (strcmp(field, "target_os") == 0) {
		arg = nxtarg(&p, " \t");
		if (*arg == '\0') {
		    fprintf(stderr, "Missing os name for target_os directive\n");
		    skip = TRUE;
		    break;
		}
		target_os_name = getenv("ode_target_os");
		if (target_os_name == NULL)
		    target_os_name = "<undef>";
		if (strcmp(arg, target_os_name) != 0) {
		    skip = TRUE;
		    break;
		}
		continue;
	    ...
	    if (strcmp(field, "target_objfmt") == 0) {
		arg= nxtarg(&p, " \t");
		if (*arg == '\0') {
		    fprintf(stderr, "Missing object format name for target_objfmt directive\n");
		    skip = TRUE;
		    break;
		}
		target_objfmt_name = getenv("ode_target_objfmt");
		if (target_objfmt_name == NULL)
		    target_objfmt_name = "<undef>";
		if (strcmp(arg, target_objfmt_name) != 0) {
		    skip = TRUE;
		    break;
		}
		continue;
    ...
    int         report_missing )                               /* error flag */
{
  ...
  if ( find_field ( contents, keyword, &fptr, 0 ) ) {
    if ( report_missing)
      ui_print ( VERROR, "%s line missing in sandbox rc file.\n", keyword );
  } /* if */
