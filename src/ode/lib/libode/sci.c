 ...
 * HISTORY
 * $Log: sci.c,v $
 * Revision 1.5.9.54  1994/04/27  21:00:29  robert
 * 	Fix concat call to use sub_project name
 * 	to find/read the sub_project's sc.conf file
 * 	if prj_read found a sub_project match from
 * 	the projects file.
 * 	[1994/04/20  19:52:45  robert]
 *
 * Revision 1.5.9.53  1994/02/24  19:37:05  marty
 * 	CR # 1062 - Bco now reports errors if it cannot overwrite files in sandbox.
 * 	[1994/02/24  19:36:40  marty]
 * 
 * Revision 1.5.9.52  1994/02/10  20:55:34  marty
 * 	CR # 1052 - bco -p should not change permissions on file.
 * 	[1994/02/10  20:54:53  marty]
 * 
 * 	another test
 * 	[1994/02/08  20:09:45  marty]
 * 
 * 	test it
 * 	[1994/02/08  20:04:06  marty]
 * 
 * Revision 1.5.9.51  1994/02/08  19:44:58  marty
 * 	CR # 917 - Change call to real_check_in_file() within sci_check_in_list2().
 * 	[1994/02/08  19:44:32  marty]
 * 
 * Revision 1.5.9.50  1994/02/04  20:30:55  damon
 * 	CR 827. Added xlog checking to sci_check_in_list2()
 * 	[1994/02/04  20:29:26  damon]
 * 
 * Revision 1.5.9.49  1994/01/31  22:18:59  marty
 * 	Merged with changes from 1.5.9.48
 * 	[1994/01/31  22:18:23  marty]
 * 
 * 	CR # 1002 - Add message in bcreate_undo printing filenames when can't undo a create.
 * 	[1994/01/31  21:13:33  marty]
 * 
 * Revision 1.5.9.48  1994/01/31  21:54:30  damon
 * 	CR 1000. Return an error if no ancestor found
 * 	[1994/01/31  19:44:15  damon]
 * 
 * Revision 1.5.9.47  1994/01/26  15:08:49  marty
 * 	CR # 978 - check outs of files are now more sensitive to corrupted RCS files.
 * 	[1994/01/26  15:08:24  marty]
 * 
 * Revision 1.5.9.46  1994/01/24  20:08:56  marty
 * 	CR # 1002 - Back out a few changes that were indtrduced in CR # 772.
 * 	bcreate should stop undoing a file if an error has been detected.
 * 	[1994/01/24  20:08:26  marty]
 * 
 * Revision 1.5.9.45  1994/01/24  17:34:33  marty
 * 	CR # 1003 - bcreate -undo  now leaves the file around in the sandbox
 * 	after removing it from source control.
 * 	[1994/01/24  17:34:04  marty]
 * 
 * Revision 1.5.9.44  1994/01/20  22:28:27  damon
 * 	Merged with changes from 1.5.9.43
 * 	[1994/01/20  22:19:59  damon]
 * 
 * 	Restored information about why no bcreate -undo was done
 * 	[1994/01/20  22:18:17  damon]
 * 
 * Revision 1.5.9.43  1994/01/20  22:12:53  marty
 * 	CR # 917 - More fixes
 * 	[1994/01/20  22:11:10  marty]
 * 
 * 	CR # 917 - Adjust for RCSfile lines
 * 	[1994/01/20  21:08:18  marty]
 * 
 * Revision 1.5.9.42  1994/01/20  00:15:42  marty
 * 	Merged with changes from 1.5.9.41
 * 	[1994/01/20  00:14:56  marty]
 * 
 * 	CR # 917 - Leave file in sandbox identical to checked in version.
 * 	[1994/01/20  00:12:00  marty]
 * 
 * Revision 1.5.9.41  1994/01/19  23:09:37  damon
 * 	CR 679. Correctly handle error returns for bad ancestry
 * 	[1994/01/19  23:08:58  damon]
 * 
 * Revision 1.5.9.40  1994/01/19  22:00:16  damon
 * 	Merged with changes from 1.5.9.39
 * 	[1994/01/19  21:59:52  damon]
 * 
 * 	CR 968. Removed server temp files during defuncts
 * 	[1994/01/19  21:58:12  damon]
 * 
 * Revision 1.5.9.39  1994/01/18  21:55:55  marty
 * 	CR # 925 - Add sci_remove_from_list().
 * 	[1994/01/18  21:55:15  marty]
 * 
 * Revision 1.5.9.38  1994/01/14  15:35:57  marty
 * 	CR # 990 - Fix for sci_read_files().  Print out error message if file
 * 	being checked out read-only is not under source control.
 * 	[1994/01/14  15:35:29  marty]
 * 
 * Revision 1.5.9.37  1994/01/06  16:25:01  marty
 * 	CR # 972 - Allow "bco -p" even if file is checked out.
 * 	[1994/01/06  16:24:24  marty]
 * 
 * Revision 1.5.9.36  1994/01/05  21:51:04  damon
 * 	Merged with changes from 1.5.9.35
 * 	[1994/01/05  21:50:42  damon]
 * 
 * 	CR 900. Changed VWARN to VERROR for missing rev
 * 	[1994/01/05  21:39:23  damon]
 * 
 * Revision 1.5.9.35  1994/01/05  20:00:30  marty
 * 	Add support for printing out label names with revision numbers during bdiffs
 * 	[1994/01/05  19:59:36  marty]
 * 
 * 	CR # 956 - Show revision numbers with 'diffing' revisions of files.
 * 	[1994/01/05  19:11:43  marty]
 * 
 * Revision 1.5.9.34  1994/01/05  17:21:33  marty
 * 	CR # 772 - bcreate -undo even if there are no revisions in RCS file.
 * 	[1994/01/05  17:21:07  marty]
 * 
 * Revision 1.5.9.33  1994/01/05  16:25:38  marty
 * 	CR # 875 - Change sci_check_in_list2() to remove _LOG_ file when finished.
 * 	[1994/01/05  16:25:08  marty]
 * 
 * Revision 1.5.9.32  1994/01/04  19:56:52  marty
 * 	Add atomic regions for calls to routines that create ,, files
 * 	[1994/01/04  19:47:15  marty]
 * 
 * Revision 1.5.9.31  1993/12/30  21:12:08  damon
 * 	CR 964. do not print file name if redirect co to stdout
 * 	[1993/12/30  21:11:49  damon]
 * 
 * Revision 1.5.9.30  1993/12/29  19:38:06  damon
 * 	CR 942. Changed VFATAL to VERROR for file writeable msg
 * 	[1993/12/29  19:37:46  damon]
 * 
 * Revision 1.5.9.29  1993/12/28  22:14:14  damon
 * 	CR 950. Added assignment of RC_OP value to rc_file
 * 	[1993/12/28  22:13:56  damon]
 * 
 * Revision 1.5.9.28  1993/12/22  22:37:08  marty
 * 	CR # 919 - Fix to sci_get_comment_leader().  You cannot leave this routine
 * 	under any circumstances without closing the odexm connection (regardless
 * 	of whether or not and invalid comment leader has been detected).  Leaving open
 * 	odexm connections will leave processes lying around that can lock up
 * 	sandboxed.
 * 	[1993/12/22  22:32:40  marty]
 * 
 * Revision 1.5.9.27  1993/12/21  20:10:48  marty
 * 	CR # 932 - bco -read now gives a FATAL error if writable files exists.
 * 	[1993/12/21  20:10:20  marty]
 * 
 * Revision 1.5.9.26  1993/12/21  19:15:53  marty
 * 	CR # 934 - Change the error message in sci_edit_files() when checking out
 * 	a file that is already writable.
 * 	[1993/12/21  19:15:16  marty]
 * 
 * Revision 1.5.9.25  1993/12/17  17:36:41  root
 * 	CR # 666 - Get rid of some warnings.
 * 	[1993/12/17  17:36:06  root]
 * 
 * Revision 1.5.9.24  1993/12/17  15:57:05  damon
 * 	CR 895. ODE now treats ./ as ./ instead of /
 * 	[1993/12/17  15:56:45  damon]
 * 
 * Revision 1.5.9.23  1993/12/16  20:37:58  damon
 * 	CR 920. Do not print created msg if file already under src cntl
 * 	[1993/12/16  20:37:26  damon]
 * 
 * Revision 1.5.9.22  1993/12/16  19:49:26  marty
 * 	CR # 724 - Added sci_copyright_init().  Removed copyright code from sci_init.
 * 	It turns out that only a small number of tools were concerned with copyright
 * 	validation.
 * 	[1993/12/16  19:48:57  marty]
 * 
 * Revision 1.5.9.21  1993/12/16  16:16:43  damon
 * 	CR 918. Added sci_select_need_merge to reduce unnecessary merging
 * 	[1993/12/16  16:16:19  damon]
 * 
 * Revision 1.5.9.20  1993/12/16  15:03:14  damon
 * 	CR 915. Added print stmts for bdiff
 * 	[1993/12/16  15:02:52  damon]
 * 
 * Revision 1.5.9.19  1993/12/15  21:21:40  marty
 * 	CR # 914 - sci_check_in_list2() now prints out the name of the file being checked in.
 * 	[1993/12/15  21:21:18  marty]
 * 
 * Revision 1.5.9.18  1993/12/10  21:05:24  marty
 * 	CR # 840 - in the routine copy_files(), ".tmp" files are no longer created
 * 	in "src" tree of a sandbox.  They are created in ${sandbox_base}/tmp.
 * 	[1993/12/10  21:04:28  marty]
 * 
 * Revision 1.5.9.17  1993/12/09  20:57:41  marty
 * 	CR # 840 - Remove debugging print statement.
 * 	[1993/12/09  20:57:09  marty]
 * 
 * Revision 1.5.9.16  1993/12/08  22:12:21  marty
 * 	CR # 840 - Remove extra printing of file name on check in.
 * 	[1993/12/08  22:11:23  marty]
 * 
 * 	 CR # 840 - Place temporary files in BCSTEMP
 * 	[1993/12/08  22:10:08  marty]
 * 
 * Revision 1.5.9.15  1993/12/08  20:26:25  marty
 * 	CR # 900 - Call to err_log needed a few more arguments.
 * 	[1993/12/08  20:26:02  marty]
 * 
 * Revision 1.5.9.14  1993/12/07  19:56:31  marty
 * 	CR # 890 - Add sci_validate_submit_set_list().
 * 	[1993/12/07  19:55:43  marty]
 * 
 * Revision 1.5.9.13  1993/12/06  21:55:51  marty
 * 	CR # 827 - Add -xlog to sci_check_in_file().
 * 	[1993/12/06  21:55:24  marty]
 * 
 * Revision 1.5.9.12  1993/12/03  19:54:40  damon
 * 	CR 886. Removed extraneous printf of ancestor info
 * 	[1993/12/03  19:37:01  damon]
 * 
 * Revision 1.5.9.11  1993/12/02  21:49:47  marty
 * 	CR # 875 - Remove _LOG_ file after checking in the file.
 * 	[1993/12/02  21:30:36  marty]
 * 
 * Revision 1.5.9.10  1993/12/02  21:10:28  damon
 * 	CR 880. Check for NULL leader prior to strcmp()
 * 	[1993/12/02  21:09:44  damon]
 * 
 * Revision 1.5.9.9  1993/12/01  20:04:13  marty
 * 	CR # 871 - bcreate with BIN or NONE comment leaders now sets keyword expansion to 'o'.
 * 	[1993/12/01  20:03:48  marty]
 * 
 * Revision 1.5.9.8  1993/11/23  21:11:14  marty
 * 	CR # 839 - Edits during the check-in phase of 'bci' are now copied back into
 * 	the users sandbox.
 * 	[1993/11/23  21:10:45  marty]
 * 
 * Revision 1.5.9.7  1993/11/22  20:48:51  marty
 * 	CR # 234 - sci_read_file() has been changed to support the "-p"option
 * 	(output to screen).
 * 	[1993/11/22  20:48:08  marty]
 * 
 * Revision 1.5.9.6  1993/11/22  20:35:05  damon
 * 	CR 679. Validate ancestry before it is ever used
 * 	[1993/11/22  20:34:06  damon]
 * 
 * Revision 1.5.9.5  1993/11/22  19:38:38  marty
 * 	CR # 815 - Fix autoflag typo.
 * 	[1993/11/22  19:37:50  marty]
 * 
 * Revision 1.5.9.4  1993/11/22  18:46:49  damon
 * 	CR 852. Separated looking up ancestry from determining ancestor
 * 	[1993/11/22  18:46:11  damon]
 * 
 * Revision 1.5.9.3  1993/11/22  17:34:15  marty
 * 	CR # 848 - match_comment_leader() now recognizes NON and BIN.
 * 	[1993/11/22  17:33:42  marty]
 * 
 * Revision 1.5.9.2  1993/11/20  20:57:16  damon
 * 	CR 790. bcreate now complains if no user branch on undo
 * 	[1993/11/19  19:37:20  damon]
 * 
 * 	CR 790. bcreate now complains if no user branch on undo
 * 
 * Revision 1.5.9.1  1993/11/16  19:16:36  marty
 * 	CR # 815 - specifying "auto" during query part of check in of many files now "stays"
 * 	in the auto state.
 * 	[1993/11/16  19:16:10  marty]
 * 
...
static char rcsid[] = "@(#)$RCSfile: sci.c,v $ $Revision: 1.5.9.54 $ (OSF) $Date: 1994/04/27 21:00:29 $";
#endif
...
void
sci_lookup_revision ( char *, char *, char *, ERR_LOG *);
...
int
sci_read_file ( char * , char * , char ** , BOOLEAN * , ERR_LOG * , BOOLEAN );
 ...
 */
{
    ...
    if (log  != OE_OK) {
	ui_print (VERROR, 
		"The file %s could not be removed from source control.\n", 
		sci_ptr->name);
        return( ERROR );
    }
    ...
    if (status != 1) {
	/* File is still there. */
	ui_print (VERROR, "The file %s could not be removed from source control.\n",
		sci_ptr->name);
        if ( sci_ptr -> ver_user == NULL ) {
          ui_print ( VCONT, "No user branch.\n" );
        } /* if */
       	return( ERROR );
    } else
	file_removed = TRUE;
    ...
    if (file_removed == TRUE)
        ui_print ( VNORMAL, "removing %s from source control.\n", working_file);
    /* if */
    return( OK );
}
...
sci_ancestor_update_list2 ( SCI_LIST sl )
{
  ...
  for ( sci_ptr = sl -> head; sci_ptr != NULL; sci_ptr = sci_ptr -> next ) {
    if ( src_ctl_get_ancestry ( sci_ptr -> name, &(sci_ptr -> ancestry) )
         != OK ) {
                ...
                /* local file copy? If FALSE, dst is on another machine */
{
  ...
  (void) concat(tmpbuf, sizeof(tmpbuf), dst, ".copy.tmp", NULL);
  strcpy (tmp, tmpfile_name (tmpbuf));
  (void) unlink(tmp);
...
src_ctl_merge ( SCI_ELEM sci_ptr )
{
  ...
  tmpdate[0] = '\0';
  tmptime[0] = '\0';
  hst_insert_file (temp_working_file, final_list, sci_ptr -> leader, NULL, tmpdate, tmptime);
...
*/
  ...
  if ( sub_project == NULL ) {
    ...
    concat ( buf2, sizeof (buf2), backing_build, "/rc_files/", project, "/", sub_project, NULL );
  } /* if */
...
/*
 * FUNCTION sci_copyright_init
 *
 * This function reads all copyright information from the 
 * backing tree. 
 * 
 */
int
...
char * sb)
{
  ...
  if ( get_rc_value ( "check_copyrights", &check_copyrights, contents, FALSE )
    ...
    if ( strcmp ( check_copyrights, "true" ) == 0 ) {
  	...
  	if ( sub_project == NULL ) {
    		concat ( copyright_file, sizeof (copyright_file), backing_build, "/rc_files/", 
			project, "/copyrights", NULL );
  	} else {
    		concat ( copyright_file, sizeof (copyright_file), backing_build, "/rc_files/", 
			project, "/", sub_project, "/copyrights", NULL );
  	} 
...
int
sci_remove_from_list ( SCI_LIST sl, SCI_ELEM sci_ptr )
{

#ifndef NO_DEBUG
  enter ( "sci_remove_from_list" );
#endif
  if ( ! confirm_alloc ( sl ) ) {
#ifndef NO_DEBUG
    leave ( );
  ...
  if (sl -> elem_cnt == 1) {
	...
	sl -> tail = NULL;
  	...
  	if (sci_ptr == sl -> head) {
		sl -> head = sci_ptr -> next;
		(sl -> head) -> prev = NULL;
  	} else if (sci_ptr == sl -> tail) {
		sl -> tail = sci_ptr -> prev;
		(sl -> tail) -> next = NULL;
  	} else {
		(sci_ptr -> prev) -> next = sci_ptr -> next;
		(sci_ptr -> next) -> prev = sci_ptr -> prev;
  	}
  }
  sl -> elem_cnt -= 1;
  if (sci_ptr -> name != NULL)
	free (sci_ptr -> name);
  free (sci_ptr);
  return ( OK );
}
 ...
 * FUNCTION sci_prev
 *
 * Give the previous element in a list from the element given.
 */
SCI_ELEM
sci_prev ( SCI_ELEM se )
{
  return ( se -> prev );
} /* end sci_prev */
  ...
  for ( sci_ptr = sl -> head; sci_ptr != NULL; sci_ptr = sci_ptr -> next) {
    ...
    if ( buf == NULL || *buf == '\0' ) {
      ...
      * missing_revs = TRUE;
      ...
      if ( sci_ptr -> leader == NULL || strcmp(sci_ptr -> leader, "BIN")
           != 0)  {
        sci_ptr -> need_merge = TRUE;
      }
    } /* if */
...
/*
 * FUNCTION sci_ancestry_list
 */
int sci_ancestry_list ( SCI_LIST sl )
{
  int status;
  SCI_ELEM sci_ptr;
  ERR_LOG log;

  enter ( "sci_ancestry_list" );
  if ( ! confirm_alloc ( sl ) ) {
    leave ();
    return ( ERROR );
  } /* end if */
 ...
 */
  if ( sci_ptr -> defunct ) {
    if ( src_ctl_check_in ( sci_ptr -> name, sci_ptr -> ver_merge,
                            "File is defunct", "Defunct", &log, checkin_date , checkin_time)
         != OK || log != OE_OK ) {
      end_atomic ( );
      if ( log == OE_OK ) {
        return ( err_log ( OE_INTERNAL ) );
    ...
    if ( sci_ptr -> has_merge_branch ) {
      if ( src_ctl_check_in ( sci_ptr -> name, sci_ptr -> ver_merge, logmsg ,
                              state, &log, checkin_date , checkin_time) != OK || log != OE_OK ) {
        end_atomic ( );
        if ( log == OE_OK ) {
          return ( err_log ( OE_INTERNAL ) );
        } /* if */
...
int sci_check_in_list2 ( SCI_LIST sl, char * user_set, char * msg_str,
                         BOOLEAN defunct, BOOLEAN condense, char * xlog_rev )
{
  SCI_ELEM sci_ptr;
  int status = OK;
  int tmp_status;
  ...
  for ( sci_ptr = sl -> head; sci_ptr != NULL; sci_ptr = sci_ptr -> next ) {
    ...
    if ( sci_has_log ( sci_ptr ) ) {
      if (condense) {
        src_ctl_lookup_revision ( sci_ptr -> name, xlog_rev, buffer, &log);
        tmp_xlog_rev = buffer;
      } else {
        tmp_xlog_rev = sci_ptr -> ver_user;
      }
      if ( ( log = hst_lookup_logmsg ( 
                          sci_ptr -> ver_user, sci_ptr -> leader,
                          condense, tmp_xlog_rev, 1 ) ) != OE_OK )
        status = ERROR;
      /* if */
    /* if */
    }
    ui_print ( VNORMAL, "%s\n", sci_ptr -> name );
    if ( condense && sci_has_log (sci_ptr) ) {
      if ( ( tmp_status = real_check_in_file ( sci_ptr, user_set, NULL,
                                                 defunct, TRUE ) ) != OK ) {
          status = tmp_status;
      } /* if */
    } else {
  ...
  log = oxm_close ( rcs_monitor );
  unlink (mesgfile);		/* Remove _LOG_ file created by hst_lookup_logmsg(). */
  leave ();
  return ( status );
} /* sci_check_in_list2 */
...
real_check_in_file ( SCI_ELEM sci_ptr, char * user_set, char * msg_str,
                     BOOLEAN defunct, BOOLEAN copy_back )
{
  int improper;
  int status;
  int process_logs;
  ...
  if ( process_logs &&
    ...
    if ( defunct ) {
      src_ctl_check_in ( sci_ptr -> name, sci_ptr -> ver_user, logmsg,
                         "Defunct", &log, checkin_date , checkin_time);
      (void) unlink ( working_file );
    } else {
      if (copy_back == TRUE) {
	   /* The temporary file has been edited before check-in.
	      Copy the edited file back into the working file. */
	 if (strcmp (working_file, sci_ptr->name) != 0)
		ui_print (VWARN, "Name conflict during checkin.  Working file is %s while checking in file %s.\n", working_file, sci_ptr->name);
	   copy_file (temp_working_file, working_file, TRUE);
      }
      status = src_ctl_check_in ( sci_ptr -> name, sci_ptr -> ver_user, 
		logmsg, "Exp", &log, checkin_date, checkin_time );
      if (status == 0) { 
	if (process_logs) {
	   next_ver_user = (char *)hst_next_revision(sci_ptr->ver_user);
      	   new_log = hst_alloc_entry (sci_ptr -> leader, logmsg, next_ver_user, checkin_date,
	   checkin_time);
      	   history = hst_xtract_file (working_file, sci_ptr->leader);
      	   new_log->next = history;
      	   hst_insert_file (working_file, new_log, sci_ptr -> leader, next_ver_user, 
  ...
  for (;;) {
    ...
    case BA_CHECKIN:
	  ...
	  unlink (mesgfile);
        ...
        if ( condense && sci_has_log (sci_ptr) ) {
          real_check_in_file ( sci_ptr, user_set, NULL, 
		               defunct, copy_back_file );
        } else {
          real_check_in_file ( sci_ptr, user_set, msg_str, 
		               defunct, copy_back_file );
        } /* if */
        log = oxm_close ( rcs_monitor );
	unlink (mesgfile);
    ...
    case BA_XTRACT:
      if ( sci_has_log ( sci_ptr ) ) {
	if (condense) {
		sci_lookup_revision (sci_ptr->name, xlog_rev, buffer, &log);
		tmp_xlog_rev = buffer;
	} else {
		tmp_xlog_rev = xlog_rev;
	}
        if ( ( log = hst_lookup_logmsg ( 
                            sci_ptr -> ver_user, sci_ptr -> leader,
                            condense, tmp_xlog_rev, 1 ) ) != OE_OK )
    ...
    case BA_DIFF:
      ...
      src_ctl_diff_rev_with_file ( sci_ptr -> ver_user, temp_working_file,
                                   sci_ptr -> name, -1, FALSE, FALSE, FALSE,
				   &log );
      log = oxm_close ( rcs_monitor );
      break;
    case BA_EDIT:
    ...
    case BA_LOGDIFF:
      ...
      /* if */
      src_ctl_diff_rev_with_file( sci_ptr -> ver_user, temp_working_file,
                                  sci_ptr -> name, fd, FALSE, FALSE, FALSE,
				  &log);
      (void) close(fd);
      log = oxm_close ( rcs_monitor );
      def = "log";
 ...
 */
ERR_LOG
sci_diff_rev_with_file ( SCI_LIST sl, BOOLEAN context, BOOLEAN whitespace, BOOLEAN unified, char * symbol_name )
{
  ERR_LOG log;
  SCI_ELEM sci_ptr;
  struct stat statbuf;
  ...
  for ( sci_ptr = sl -> head; sci_ptr != NULL; sci_ptr = sci_ptr -> next ) {
	...
	if (status != 0) {
		ui_print (VWARN, 
    		...
    		ui_print ( VNORMAL, "Comparing the file in your sandbox with revision %s",
			sci_ptr -> ver_generic1);
		if (symbol_name != NULL)
			ui_print ( VNORMAL, " [%s]", symbol_name);
		ui_print ( VNORMAL, ".\n");
    		src_ctl_diff_rev_with_file ( sci_ptr -> ver_generic1, real_file,
                                             sci_ptr -> name, -1, context,
                                             whitespace, unified, &log );
	}
 ...
 */
ERR_LOG
sci_diff_rev_with_rev ( SCI_LIST sl, BOOLEAN context, BOOLEAN whitespace, BOOLEAN unified, char * symbol_comparor,
char * symbol_comparee )
{
  ...
  for ( sci_ptr = sl -> head; sci_ptr != NULL; sci_ptr = sci_ptr -> next ) {
	...
	continue;
    	...
    	check_path ( sci_ptr -> name );
        ui_print ( VNORMAL, "%s\n", sci_ptr -> name );
	ui_print ( VNORMAL, "Comparing revision %s ",
		sci_ptr->ver_generic1);
	if (symbol_comparor != NULL)
		ui_print ( VNORMAL, "[%s] ", symbol_comparor);
	ui_print ( VNORMAL, "with revision %s",
		sci_ptr->ver_generic2);
	if (symbol_comparee != NULL)
		ui_print ( VNORMAL, " [%s]", symbol_comparee);
        ui_print ( VNORMAL, ".\n" );

    	src_ctl_show_diffs ( sci_ptr -> ver_generic1, sci_ptr -> ver_generic2,
                         sci_ptr -> name, context, whitespace, unified, &log );
  } /* for */
...
sci_rm_submit ( SCI_LIST file_set, char * set_name )
{
  ...
  for ( sci_ptr = file_set -> head; sci_ptr != NULL; sci_ptr = sci_ptr-> next) {
    ...
    if ( src_ctl_check_in ( sci_ptr -> name, sci_ptr -> ver_latest, logmsg ,
                            "Exp", &log, checkin_date, checkin_time ) != OK ) {
       e_log = log;
    } /* if */
...
int
sci_read_files ( SCI_LIST sl, char * revision, BOOLEAN to_screen)
{
  SCI_ELEM sl_elem;
  char * selected_rev;
  ERR_LOG log;
  ...
  while ( sl_elem != NULL ) {
    ...
    if ( (to_screen == FALSE) && (access (sl_elem->name, F_OK) == 0 ) && 	
		((st.st_mode & S_IWUSR) && ! is_link )) {
        ui_print ( VFATAL, "File %s is writeable.\n", sl_elem -> name );
	ui_print ( VCONT, 
			"This file may already be checked out for editing.\n");
        ui_print ( VCONT, "Check out read-only will not be performed for this file.\n" );
    } else {
    	if (sci_read_file ( sl_elem->name, revision, 
		&selected_rev, &defunct, &log, to_screen) == OK) {
    	   if ( log == OE_OK ) { 
      		if ( ! to_screen ) {
      		  stat ( sl_elem->name, &st );
      		  chmod ( sl_elem->name, 
...
sci_read_file ( char * pathname, char * rev_str, char ** selected_rev,
                BOOLEAN * defunct, ERR_LOG * log, BOOLEAN to_screen )
{
  int i;
  const char *av[16];
  char *rev_arg;
 ...
 */
   if ( src_ctl_lookup_revision ( pathname, rev_str, revision, log ) != 0 ) {
     if (src_ctl_file_exists (pathname, &erlog) != 0) {
        ui_print (VERROR,
                "File %s is not under source control.\n",
                pathname);
	*log = err_log ( OE_OPEN, pathname, NULL );
     } else {
	if (is_revision (rev_str) == TRUE) {
     		ui_print ( VERROR, 
		   "File does not contain the revision specified. \n" );
     		ui_print ( VCONT, "File: %s\n", pathname );
     		ui_print ( VCONT, "Revision: %s\n", rev_str );
     		*log = err_log ( OE_REVISION, rev_str, pathname );
	} else {
     		ui_print ( VERROR, 
		   "File does not contain a revision for the label (or labels) specified. \n" );
     		ui_print ( VCONT, "File: %s\n", pathname );
     		ui_print ( VCONT, "Label string: %s\n", rev_str );
     		*log = err_log ( OE_REVISION, "", pathname );
	}
     }
     return (OK);
   } /* if */
...
*/
   ...
   *log = oxm_endcmd ( rcs_monitor, &status );
   ...
   if (status != 0) {
	ui_print (VERROR,
		"An error has occurred while checking out file %s\n",
		pathname);
	return (ERROR);
   }
...
ERR_LOG
sci_select_need_merge ( SCI_LIST file_set, SCI_LIST * files_need_merge )
{
  SCI_ELEM sci_ptr;
  SCI_ELEM sci_tmp;

  sci_new_list ( files_need_merge );
  for ( sci_ptr = sci_first ( file_set ); sci_ptr != NULL;
        sci_ptr = sci_next ( sci_ptr ) )
    if ( sci_ptr -> need_merge ) {
      ...
      memcpy ( (void *)sci_tmp, (void *)sci_ptr,
                sizeof ( struct sci_elem ) );
      if ( (*files_need_merge) -> tail == NULL )
        (*files_need_merge) -> head = sci_tmp;
      else
        ( (*files_need_merge) -> tail ) -> next = sci_tmp;
      /* end if */
      (*files_need_merge) -> tail = sci_tmp;
      (*files_need_merge) -> elem_cnt += 1;
    } /* if */
  /* for */
  return ( OE_OK );
} /* end sci_select_need_merge */
...
sci_edit_files ( SCI_LIST sl, char * symbolic_name, char * config_str )
{
  ...
  if ( ! confirm_alloc ( sl ) )
    return ( ERROR );
  /* end if */
   set_and_log_kludge ( symbolic_name );
   sl_elem = sci_first ( sl );
   log = oxm_open ( &rcs_monitor, RCS_MONITOR );
   while ( sl_elem != NULL ) {
     check_path ( sl_elem -> name );
     ...
     if ( (access (sl_elem->name, F_OK) == 0 ) && 	
		((st.st_mode & S_IWUSR) && ! is_link )) {
        ui_print ( VERROR, "File %s is writeable.\n", sl_elem -> name );
	if (set_lookup (symbolic_name, sl_elem->name) == FALSE) {
		ui_print ( VCONT, 
			"This file is not part of the current set %s.\n",
			symbolic_name);
	}
        ui_print ( VCONT, "Check out will not be performed for this file.\n" );
	...
	if (is_link) {
		/* If a symbolic link exists, make sure that it 
		   can be overwritten. */
		if (unlink (sl_elem->name) != 0) {
			ui_print ( VERROR, 
				"Unable to overwrite symbolic link %s.\n", 
				sl_elem->name);
     			sl_elem = sci_next ( sl_elem );
			continue;
		}
...
*/
	      ...
	      if (sci_get_comment_leader( sl_elem->name, leader ) != ERROR) {
		      if (strcmp(leader, "BIN") == 0 ||
			  strcmp(leader, "NONE") == 0) {
			      av[i++] = "-ko";
			      tempslot [0]  = '4';
		      }
	      }
		...
		ui_print (VERROR, 
			"An error has occurred during check-out of file %s\n",
			sl_elem->name);
  	      	tmpfile_delete ( sl_elem->name );
             	sl_elem = sci_next ( sl_elem );
             	continue;
	   ...
	   if (sci_read_file ( sl_elem->name, config_str, &selected_rev,
                           &defunct, &log, FALSE) != OK) {
             	sl_elem = sci_next ( sl_elem );
             	continue;
	   }
	   ...
	   free(tempfile);
           ...
           ui_print ( VDETAIL, "Creating branch for user from ancestor %s\n",
                               selected_rev );
           if (src_ctl_create_branch ( sl_elem -> name, 
			trunk_rev, symbolic_name, selected_rev, 
			NULL, &log ) == ERROR) {
			ui_print (VERROR,
			   "An error has occurred during check-out of file %s\n",
			   sl_elem->name);
...
match_comment_leader (char * comment_leader, struct rcfile rc_info)
{
   ...
   if ((strcmp (comment_leader, "NONE") == 0) ||
	(strcmp (comment_leader, "BIN") == 0)) {
	return (TRUE);
   }
...
/*
 * FUNCTION	sci_lookup_revision()
 *
 * Given a label or branch revision for a file, return the
 * tip revision on that branch.
 *
 */
void
sci_lookup_revision ( char * filename, char *label,
	char *revision, ERR_LOG *log)
{
   ...
   src_ctl_lookup_revision (filename, label, buffer, &local_log);
...
/*
 * FUNCTION sci_validate_submit_set_list
 *
 * Verify that the submit_set (if it exists) on each file points to a branch. 
 */
BOOLEAN
sci_validate_submit_set_list ( SCI_LIST sl , char * submit_set )
{
  int status = OK, ret;
  BOOLEAN keep_looping;
  SCI_ELEM sci_ptr;
  ...
  ui_print ( VDEBUG, "Entering sci_validate_submit_set_list.\n" );
  if ( ! confirm_alloc ( sl ) )
    return ( ERROR );
  /* if */
  ...
  rev_sw  = alloc_switch ('b', submit_set);
  file_sw = alloc_switch ('f', tempfile_in);
  ...
  for ( sci_ptr = sci_first ( sl ); 
    ...
    if (strcmp (file, sci_ptr->name) == 0) {
	if ( strcmp(revision, "(null)") != 0 ) { 
		if (is_branch (revision) == FALSE) {
			ui_print ( VFATAL, 
				"Error in %s.  Cannot submit to the set %s.\n", 
				sci_ptr->name, submit_set);
			if (is_revision(revision) == TRUE) {
				ui_print ( VCONT, 
					"The symbolic name %s is bound to the revision %s.\n", 
					submit_set, revision);
			} else {
				ui_print ( VCONT, 
					"The symbolic name %s is bound to the string %s.\n", 
					submit_set, revision);
			}
           		status = ERROR;
		}
	...
	/* This is probably due to a file not having a revision to match
	   the revision string (i.e. the file doesn't have that revision).
	   Skip this element in the "sl" list. */
	ui_print (VFATAL, "Cannot validate set %s for file %s.\n",
		submit_set, sci_ptr->name);
	sci_ptr = sci_next(sci_ptr); 
	status = ERROR;
    } else {
	if (fgets (buffer, sizeof(buffer), tempfile_out_fd) != NULL) {
  ...
  ui_print ( VDEBUG, "Leaving sci_validate_submit_set_list.\n" );
  if (status == ERROR)
	return (FALSE);
  else
  	return (TRUE);
