 ...
 * HISTORY
 * $Log: src_ctl_rcs.c,v $
 * Revision 1.1.10.20  1994/02/22  16:04:41  damon
 * 	CR 1049. Pedantic changes
 * 	[1994/02/22  16:04:22  damon]
 *
 * Revision 1.1.10.19  1994/02/08  19:46:52  marty
 * 	CR # 917 - Remove forcing date on check-in.
 * 	[1994/02/08  19:33:51  marty]
 * 
 * Revision 1.1.10.18  1994/01/31  19:47:27  damon
 * 	CR 1000. Return an error if no ancestor found
 * 	[1994/01/31  19:45:50  damon]
 * 
 * Revision 1.1.10.17  1994/01/28  23:31:30  damon
 * 	CR 1020. Simplified code and extended ancestry determination
 * 	[1994/01/28  23:31:09  damon]
 * 
 * 	checkpoint 1
 * 	[1994/01/28  23:18:08  damon]
 * 
 * Revision 1.1.10.16  1994/01/24  20:10:50  root
 * 	CR 756. Put nul at end of branch string
 * 	[1994/01/24  20:10:33  root]
 * 
 * Revision 1.1.10.15  1994/01/20  22:13:25  marty
 * 	Merged with changes from 1.1.10.14
 * 	[1994/01/20  22:12:25  marty]
 * 
 * 	CR # 917 - More fixes
 * 	[1994/01/20  22:11:23  marty]
 * 
 * 	CR # 917 - Adjust for RCSfile lines
 * 	[1994/01/20  21:08:30  marty]
 * 
 * Revision 1.1.10.14  1994/01/20  19:07:29  damon
 * 	CR 1000. Check for two field rev in ancestor comparisons
 * 	[1994/01/20  19:07:12  damon]
 * 
 * Revision 1.1.10.13  1994/01/20  16:38:58  marty
 * 	Clean up warning messages.
 * 	[1994/01/20  16:38:44  marty]
 * 
 * Revision 1.1.10.12  1994/01/20  00:16:19  marty
 * 	Merged with changes from 1.1.10.11
 * 	[1994/01/20  00:15:06  marty]
 * 
 * 	CR # 917 - Leave file in sandbox identical to checked in version.
 * 	[1994/01/20  00:12:13  marty]
 * 
 * Revision 1.1.10.11  1994/01/19  23:09:29  damon
 * 	CR 679. Correctly handle error returns for bad ancestry
 * 	[1994/01/19  23:09:09  damon]
 * 
 * Revision 1.1.10.10  1994/01/05  13:55:59  damon
 * 	CR 884. Tell rcs not to print information during ancestry update
 * 	[1994/01/05  13:55:51  damon]
 * 
 * Revision 1.1.10.9  1993/12/16  21:10:31  damon
 * 	CR 921. Added BADUNDOREV ode errno
 * 	[1993/12/16  21:10:13  damon]
 * 
 * Revision 1.1.10.8  1993/12/07  19:56:46  marty
 * 	CR # 890 - Add is_branch();
 * 	[1993/12/07  19:56:10  marty]
 * 
 * Revision 1.1.10.7  1993/12/03  19:54:23  damon
 * 	CR 884. Added -q to rcsdiff calls
 * 	[1993/12/03  19:37:17  damon]
 * 
 * Revision 1.1.10.6  1993/12/02  17:39:18  damon
 * 	CR 880. Tell rcs to be quiet when outdating for bcreate -undo
 * 	[1993/12/02  17:39:09  damon]
 * 
 * Revision 1.1.10.5  1993/11/23  21:57:28  damon
 * 	CR 679. Fixed done condition in valid_ancestry_rev()
 * 	[1993/11/23  21:57:20  damon]
 * 
 * Revision 1.1.10.4  1993/11/22  20:35:17  damon
 * 	CR 679. Validate ancestry before it is ever used
 * 	[1993/11/22  20:34:22  damon]
 * 
 * Revision 1.1.10.3  1993/11/22  17:57:17  damon
 * 	CR 763. Relay server output from bmerge_s and branch_ci
 * 	[1993/11/22  17:57:08  damon]
 * 
 * Revision 1.1.10.2  1993/11/19  21:26:58  damon
 * 	CR 844. Fixed ancestry comparisons
 * 	[1993/11/19  21:26:44  damon]
 * 
 * Revision 1.1.10.1  1993/11/19  19:33:55  damon
 * 	CR 790. bcreate now complains if no user branch on undo
 * 	[1993/11/19  19:25:18  damon]
 * 
...
ERR_LOG
src_ctl_undo_create( const char * name, const char * rev )
{
  int i;
  const char *av[16];
  int status;
  ...
  ERR_LOG log;
    ...
    if (strcmp(rev, "1.1.1.1") != 0) {
        return( err_log ( OE_BADUNDOREV, rev ) );
    }
...
int src_ctl_show_diffs( const char *rev, const char *altrev,
                        const char * rcs_file,
                        BOOLEAN context, BOOLEAN whitespace,
		        BOOLEAN unified, ERR_LOG * log )
{
  int i;
  const char *av[16];
                     ...
                     int * min_depth )
{
  ...
  if ( numfields ( ver_user, &log ) == 2 ) {
    user_is_trunk = TRUE;
    user_rev = strdup ( ver_user );
  } else {
    user_rev = get_branch ( ver_user );
  } /* if */
  if ( numfields ( ver_merge, &log ) == 2 ) {
    merge_is_trunk = TRUE;
    merge_rev = strdup ( ver_merge );
  } else {
    merge_rev = get_branch ( ver_merge );
  } /* if */
  ...
  } else if ( strcmp ( user_rev, merge_rev ) == 0 ) {
    free ( user_rev );
    free ( merge_rev );
  ...
  for (;;) {
    ...
    if ( numfields ( successor, &log ) == 2 ) {
      successor_rev = strdup ( successor );
    } else {
      successor_rev = get_branch ( successor );
    } /* if */
    if ( strcmp ( successor_rev, user_rev ) == 0 ) {
      found = TRUE;
      ver_search = ver_merge;
      if ( getancestor1 ( ancestor, ver_search, r3, ancestry, depth2+1,
                          depth1, min_depth ) ) { 
      } /* if */
    } /* if */
    free ( successor_rev );
  ...
  for (;;) {
    ...
    if ( strcmp ( successor_rev, merge_rev ) == 0 ) {
      found = TRUE;
      ver_search = ver_user;
      found = getancestor1 ( ancestor, ver_search, r3, ancestry, depth1+1,
                             depth2, min_depth );
    } /* if */
    free ( successor_rev );
  } /* for */
...
int src_ctl_check_in ( const char * name, const char * rev,const char * logmsg,
                       const char * state, ERR_LOG * log, char checkin_date[], 
		       char checkin_time[] )
{
  int i;
  char *p, *lastdot;
 ...
 */
  ...
  gm_get_date (gdate, gtime);
  concat (ch_date, sizeof(ch_date), gdate, " ", gtime, NULL);
  strcpy (checkin_date, gdate);
  strcpy (checkin_time, gtime);
  av[i++] = "ci";
  av[i++] = "-f";
  av[i++] = "-q";
  av[i++] = state_sw;
...
#define revlength 30
  ...
  for (;;) {
    if ( set_lock2 ( comma_v_file, &fd ) == WAITING ) {
      ui_print ( VNORMAL,
                 "File temporarily locked for ancestry update. Waiting.\n" );
      sleep ( 5 );
    } else {
      break;
    } /* if */
...
BOOLEAN valid_ancestry_rev ( char * rev )
{
  ERR_LOG log;
  int nf;

  if ( rev == NULL || *rev == '\0' ) {
    ui_print ( VNORMAL, "null rev\n" );
    return ( FALSE );
  } /* if */
  nf = numfields ( rev, &log );
  if ( log != OE_OK ) {
    return ( FALSE );
  } /* if */
  return ( nf == 2 || nf == 4 );
} /* end valid_ancestry_rev */

BOOLEAN valid_ancestry ( char * ancestry )
{
  char * successor;
  char * ancestor;
  char * a_ptr;
  ...
  while ( *( successor = nxtarg (&a_ptr, ">" ) ) != '\0' ) {
    if ( !valid_ancestry_rev ( successor ) ) {
      valid = FALSE;
      break;
    } /* if */
    ancestor = nxtarg (&a_ptr, ";" );
    if ( !valid_ancestry_rev ( ancestor ) ) {
      valid = FALSE;
      break;
    } /* if */
  } /* while */
...
src_ctl_get_ancestry ( char * rcs_file, char ** ancestry )
{
    ...
    get_ancestry ( ancestry, &log );
    log = oxm_endcmd( rcs_monitor, &status );
    if ( ! valid_ancestry ( *ancestry ) ) {
      ui_print ( VFATAL, "Corrupted ancestry information.\n" );
...
if (strstr(branch, ".") == NULL) return (FALSE);

ptr = branch;

for (ptr = branch; *ptr != '.' && *ptr != '\0';) {
        if ((*ptr < '0') || (*ptr > '9'))
                return (FALSE);
	ptr++;
}
